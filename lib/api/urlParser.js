    ...
    for (let k in options) {
      ...
      switch (optionConfig.type) {
        ...
        case Boolean:
          ...
          if (option == false)
            args.push(optionConfig.queryPattern.replace("STATE", "exclude"));
          else if (option)
            args.push(optionConfig.queryPattern.replace("STATE", "include"));
          break;
        ...
        case Array:
          ...
          for (let i in option) {
            ...
            if (type == "INDEX") {
              ...
              if (!optionConfig.data.includes(value.toLowerCase())) continue;
              console.log(2);
              args.push(
                optionConfig.queryPattern
                  .replace("STATE", isExcluded ? "exclude" : "include")
                  .replace(
                    "INDEX",
                    optionConfig.data.indexOf(value.toLowerCase()) + 1
                  )
              );
            } else if (type == "VALUE") {
              //check if value exist in the object
              if (!optionConfig.data[value.toLowerCase()]) continue;
              args.push(
                optionConfig.queryPattern
                  .replace("STATE", isExcluded ? "exclude" : "include")
