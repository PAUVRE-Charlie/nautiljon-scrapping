module.exports = {
  searchScraper($, genre, totalLength) {
    ...
    //         ? "https://nautiljon.com" +
    //           $(e).find(".image img").attr("src").replace("imagesmin", "images")
    //         : null,
    //       description: $(e).find(".left.vtop p").text()
    //         ? $(e).find(".left.vtop p").text().replace(" Lire la suite", "")
    //         : null,
    ...
    //   });
    if (genre === "manga") {
      if (!$("tbody")[1] || !$("tbody")[1].children) return [];
  ...
  dataPageScraper($, url) {
    ...
    //           ? "https://nautiljon.com/" + $(e).find("a").attr("href")
    //           : null,
    //         relationType:
    ...
    //           ? "https://nautiljon.com/" +
    //             $(e).find("img").attr("src").replace("imagesmin", "images")
    //           : null,
    ...
    //       $("span")
    //         .filter(function () {
    //           return $(this).text().indexOf("Titre original : ") > -1;
    //         })
    //         .parent()
    //         .html()
    //         ?.split("</span>")[1]
    ...
    //       $("span")
    //         .filter(function () {
    //           return $(this).text().indexOf("Studio d'animation : ") > -1;
    //         })
    //         .parent()
    ...
    //     vodPlatform: $("span")
    //       .filter(function () {
    //         return $(this).text().indexOf("Simulcast / streaming : ") > -1;
    //       })
    //       .parent()
    //       .find("a")
    //       .toArray()
    ...
    //     description:
    //       $(
    //         $(".description").html()?.split('<div class="groupe"')[0].trim()
    //       ).text() || null,
    //     pictures: $("h3")
    //       .filter(function () {
    ...
    //               frenchName:
    //                 $(frenchNamesColumn[i]?.children[2]).text() || null,
    ...
    //               : null,
    //             date:
    //               $(e).find(".sim[href]").text()?.split(":")[0]?.trim() || null,
    //             description: $(e).find(".introNews").text() || null,
    //             imageUrl: $(e).find("img")
    //               ? "https://nautiljon.com" + $(e).find("img").attr("src")
    //               : null,
    //           };
    //         }),
    //     },
    ...
    if (genre === "mangas") {
      ...
      return {
        name: $('.h1titre > [itemprop="name"]')
          ? $('.h1titre > [itemprop="name"]').text()
          : null,
        japName: $("span")
          .filter(function () {
            return $(this).text().indexOf("Titre original : ") > -1;
        ...
        imageUrl: $(".image_fiche.fleft a img").attr("src")
          ? `https://nautiljon.com${$(".image_fiche.fleft a img")
              ...
              .replace("/mini", "")}`
          : null,
        country: $(".flag").parent().text()
          ? $(".flag").parent().text().split(":")[1].split("-")[0].trim()
          : null,
        type:
          $("span")
            .filter(function () {
        ...
        status: $("span")
          .filter(function () {
            return $(this).text().indexOf("Nb volumes VO : ") > -1;
          })
          .parent()
          .html()
          ? $("span")
        ...
        author: {
          ...
          art:
            $('li [itemprop="illustrator"] [itemprop="name"]').text() ||
        ...
        editor: {
          VO:
            $("span")
              .filter(function () {
                return $(this).text().indexOf("Éditeur VO : ") > -1;
              })
              .parent()
              .find('[itemprop="legalName"]')
              .text() || null,
          VF:
            $("span")
              .filter(function () {
                return $(this).text().indexOf("Éditeur VF : ") > -1;
              })
              .parent()
              .find('[itemprop="legalName"]')
              .text() || null,
              ...
              return {
                ...
                url: $(e).find(".sim[href]")
                  ? "https://nautiljon.com" +
                    $(e).find(".sim[href]").attr("href")
                  : null,
                date: $(e).find(".sim[href]").text()
                  ? $(e).find(".sim[href]").text().split(":")[0].trim()
                  : null,
                description: $(e).find(".introNews").text() || null,
                imageUrl: $(e).find("img")
                  ? "https://nautiljon.com" + $(e).find("img").attr("src")
